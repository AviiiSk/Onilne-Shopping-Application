...
public class CartServiceImp implements CartService {
	...
	public CartServiceImp(Connection con) {
		this.con = con;
	}
...
//		String query1 = "UPDATE product SET product_name=?,product_description=?, product_price=? WHERE product_id = ?";
		try {
			...
			if (!findCart.next()) {
				throw new ShopifyException(MessageProperties.CART_NOT_FOUND.getMessage());
			}
...
//			ResultSet cartFromDb = findCartByProductId(cart.getProductId());
			...
			if (!isProductPresent) {
				throw new ShopifyException(MessageProperties.PRODUCT_NOT_FOUND.getMessage());
			}
			state = con.prepareStatement(query);
	...
	@Override
	public int increaseQtyAndPriceInCartWhenProductIsAddedInCart(String token, cart cart, int cartId) {
