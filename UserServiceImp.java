...
public class UserServiceImp implements UserService {
	...
	public boolean createUserRegistration(User user) {
		...
		try {
			if(findUserByEmail(user.getUserEmail()).next()){
				throw new ShopifyException("This User Name is already registerd, Please Login!", 
						ShopifyException.ExceptionType.USER_ALREADY_PRESENT);
			}
	...
	public String loginUser(UserLogin userLogin) {
		...
		try {
			...
			if(!userByEmail.next()) {
				throw new ShopifyException("User with this email is not registered, Please signUp First ", ShopifyException.ExceptionType.EMAIL_NOT_FOUND);
			}
			boolean password = userLogin.getUserPass().equals(userByEmail.getString(1));
			if(!password) {
				throw new ShopifyException("Incorrect password", ShopifyException.ExceptionType.PASSWORD_INCORRECT);
			}
